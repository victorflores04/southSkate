...
export default function Estimate(){
    ...
    const navigationPreviusDisabled= ()=>{
    ...
    const hanldeSelect = (id) =>{
        const newQuestions = cloneDeep(questions);
        const currentlyActive = newQuestions.filter(question=> question.active);
        const activeIndex= currentlyActive[0].id - 1;

        const newSelected = newQuestions[activeIndex].options[id-1]
        const previusSelected = currentlyActive[0].options.filter(option=>option.selected)
       

        switch(currentlyActive[0].subtitle){
            case 'Select one.':
                if(previusSelected[0]){
                    previusSelected[0].selected=!previusSelected[0].selected
                } 
        ...
        <Grid container direction="row">
            ...
            <Grid item container direction="column" alignItems="center" lg style={{marginRight:"2em", marginBottom:"25em"}}>
                {questions.filter(question => question.active).map((question, index)=> (
                    ...
                    <Grid item>
                        <Typography variant="h2" align="center" style={{fontweight:500,fontSize:"2.25rem", marginTop:"3.5em", lineHeight: 1.25}} >
                        {question.title}
                        </Typography>
                    ...
                    <Grid item container>
                        ...
                        <Grid item container direction="column" md component={Button} onClick={()=>hanldeSelect(option.id)} style={{display:"grid", textTransform:"none",borderRadius:0 , backgroundColor:option.selected?theme.palette.common.orange:null}}>
                ...
                <Grid  item container justify="space-between" style={{width:"18em", marginTop:"3em"}}>
                    <Grid item>
                        <IconButton disabled={navigationPreviusDisabled()} onClick={previusQuestion}>
                        <img src={navigationPreviusDisabled()? backArrowDisabled:backArrow} alt="Previus question"/>
                        </IconButton>
                    </Grid>
                    <Grid item>
